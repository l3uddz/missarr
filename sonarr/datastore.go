...
const sqlUpsert = `
INSERT INTO series (series, air_date, search_date)
VALUES (?, ?, ?)
...
func (store *datastore) upsert(tx *sql.Tx, series int, airDate time.Time, searchDate *time.Time) error {
	_, err := tx.Exec(sqlUpsert, series, airDate, searchDate)
	return err
}
...
func (store *datastore) Upsert(series []Series) error {
	...
	for _, s := range series {
		if err = store.upsert(tx, s.Id, s.AirDate, s.SearchDate); err != nil {
